...
pacman::p_load(knitr, rmdformats, sf, raster, rgdal, tidyverse)
...
> One file is a layer in this instance
> One forest is a polygon, which is one feature
> The answer is the number of rows in each df
...
forests$geo_area <- st_area(forests)

# Create a quick histogram of the areas using hist
hist(forests$geo_area)

# Filter to forests greater than 30000 (square meters)
big_forests <- forests %>% filter(unclass(geo_area) > 30000)

# Can you plot the forests with their names?
plot(big_forests$geometry)
plot(big_forests["navn"]) # legend only shows up on some pc's

# Plot just the geometry of big_forests
plot(st_geometry(big_forests), lab = big_forests$navn)
```
...
plot(forests_geo, col = "green");plot(parks$geometry, col = "red", add = T)
...
> Fifth layer might be weird? Does it really have 4 layers Adela?
